...
class Network:
    ...
    def plot_results(self, hop):
        ...
        plt.plot(times, errors_new, label=f'Time Error at Hop {hop}')
        plt.xlabel('Time (s)')
...
def process_tuple(input_tuple):
    base_value = SYNC_INTERVAL * 10_000_000  # 根据同步间隔计算基准值
    processed_list = [round(x - base_value, 3) for x in input_tuple]
...
def save_tuple_to_csv(tuple_data, filename=r'D:\06_engineering\03_analysis\pj_gptp_simulation\old_version\main_test4\output_data/data_20250413v1.csv'):
    # 检查文件是否存在
...
if __name__ == "__main__":
    ...
    network.plot_results(hop=25)
    network.plot_results(hop=50)
    network.plot_results(hop=75)
