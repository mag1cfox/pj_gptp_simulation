...
**************************************
*  @Author  ：   mag1cfox
*  @Time    ：   2025/4/10 15:08
*  @Project :   pj_gptp_simulation
*  @Description :   添加每一跳的导出记录
*  @FileName:   main_test20250410claudeV1.py
**************************************
"""
...
def export_hop_data_to_csv(results):
    """
    Export time deviation data for each hop (1-100) to a CSV file.
    ...
    # 选择要导出的节点（跳数）
    nodes_to_export = list(range(1, 101))  # 1到100跳

    # 创建一个字典来存储每个节点的数据，按时间对齐
    time_aligned_data = {}
    all_times = set()

    # 收集所有的时间点和对应的数据
    for node_id in nodes_to_export:
        if node_id in results["node_deviations_time_series"]:
            time_series = results["node_deviations_time_series"][node_id]
            time_aligned_data[node_id] = {t: d for t, d in time_series}
            all_times.update(t for t, _ in time_series)
    ...
    for t in all_times:
        row = []
        for node_id in nodes_to_export:
            if node_id in time_aligned_data and t in time_aligned_data[node_id]:
                # 将偏差转换为微秒并添加到行
                deviation_us = time_aligned_data[node_id][t] * 1e6
                row.append(deviation_us)
            else:
                # 如果在这个时间点没有数据，用NaN填充
                row.append(np.nan)

        # 只有当行中至少有一个非NaN值时才添加该行
    ...
    with open(r'output_data\hop_data.csv', 'w', newline='') as csvfile:
        ...
        # 写入标题行（列名）
        headers = [f'Hop_{i}' for i in nodes_to_export]
    ...
    print(f"Exported hop data to hop_data.csv with {len(csv_data)} timestamps")
...
if __name__ == "__main__":
    ...
    # export_hop_data_to_csv(results)
