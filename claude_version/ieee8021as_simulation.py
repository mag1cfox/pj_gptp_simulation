...
class TimeAwareSystem:
    ...
    def __init__(self, node_id, is_grandmaster=False, priority1=248, priority2=248,
                 phy_jitter_ns=8, two_step_sync=True, two_step_pdelay=True):
        """
        初始化时间感知系统
    ...
    def forward_sync(self, sync_message, perfect_time, reception_time):
        ...
        for port in self.ports:
            if port.state == PortState.MASTER:
                # 创建一个新的Sync消息
                port.sync_sequence_id += 1
                debug_print(f"节点{self.node_id}通过端口{port.port_number}转发Sync消息, seq={port.sync_sequence_id}")
    ...
    def forward_follow_up(self, follow_up_message, perfect_time, reception_time):
        ...
        for port in self.ports:
            if port.state == PortState.MASTER and port.sync_sequence_id in port.sync_send_times:
                debug_print(f"节点{self.node_id}通过端口{port.port_number}转发Follow_Up消息")
                # 创建一个新的Follow_Up消息
...
class IEEE8021ASSimulation:
    ...
    def __init__(self, num_nodes=100, simulation_time=600.0, two_step_sync=True):
        """
        初始化仿真
...
if __name__ == "__main__":
    ...
    # 运行两步法仿真
    print("\n开始两步法仿真...")
    two_step_sim = IEEE8021ASSimulation(num_nodes=100, simulation_time=600.0, two_step_sync=True)
    ...
    # 运行单步法仿真
    print("\n开始单步法仿真...")
    one_step_sim = IEEE8021ASSimulation(num_nodes=100, simulation_time=600.0, two_step_sync=False)
