...
class Clock:
    ...
    def __init__(self, node_id=0, initial_time=None, initial_drift_rate=None, max_drift_ppm=10, granularity_ns=8,
        ...
        if is_ideal:
            # 理想时钟无漂移
            self.drift_rate = 0.0
            self.drift_change_rate = 0.0
        elif initial_drift_rate is None:
            ...
            self.drift_change_rate = random.uniform(-0.5, 0.5) * 1e-9

        self.granularity = granularity_ns * 1e-9  # 时钟粒度（秒）

        status = "理想时钟" if is_ideal else "普通时钟"
        debug_print(f"初始化节点{node_id}的{status}: 初始时间={self.local_time}秒, 漂移率={self.drift_rate * 1e6} ppm")
    ...
    def update(self, elapsed_perfect_time):
        ...
        if self.is_ideal:
            # 理想时钟与完美时间同步
            self.local_time += elapsed_perfect_time
    ...
    def set_time(self, new_time):
        """设置本地时间"""
        if not self.is_ideal:  # 理想时钟不应该被设置
...
class Port:
    ...
    def receive_message(self, message, perfect_time):
        ...
        # 调试输出，确认消息处理
        message_type_str = message.message_type.name if hasattr(message.message_type, 'name') else str(
...
class TimeAwareSystem:
    ...
    def __init__(self, node_id, is_grandmaster=False, priority1=248, priority2=248, phy_jitter_ns=8):
        ...
        # 使用节点ID初始化时钟（用于初始化）
        if is_grandmaster:
            # GM时钟应该是理想的（无漂移，精确的时间）
            self.clock = Clock(node_id=node_id, initial_time=0.0, initial_drift_rate=0.0, is_ideal=True)
        else:
            # 非GM时钟有随机初始偏差和漂移
        ...
        status = "主时钟节点" if is_grandmaster else "普通节点"
        debug_print(f"初始化时间感知系统: {status} {node_id}")
    ...
    def forward_sync(self, sync_message, perfect_time, reception_time):
        ...
        for port in self.ports:
            if port.state == PortState.MASTER:
                ...
                port.sync_sequence_id += 1
                debug_print(
                    f"节点{self.node.node_id}通过端口{port.port_number}转发Sync消息, seq={port.sync_sequence_id}")
...
class IEEE8021ASSimulation:
    ...
    def _process_message_event(self, port, message):
        ...
        if new_events:
            ...
            for event in new_events:
                ...
                new_message_type = new_message.message_type.name if hasattr(new_message.message_type, 'name') else str(
                    new_message.message_type)
                debug_print(
                    f"  -> 发送{new_message_type}消息到节点{target_port.node.node_id}端口{target_port.port_number}，到达时间={arrival_time:.6f}")
    ...
    def _process_initiate_sync(self, params):
        ...
        if sync_events:
            ...
            for event in sync_events:
                ...
                message_type = message.message_type.name if hasattr(message.message_type, 'name') else str(
                    message.message_type)
                debug_print(
                    f"  -> 发送{message_type}消息到节点{target_port.node.node_id}端口{target_port.port_number}，到达时间={arrival_time:.6f}")
    ...
    def _analyze_results(self):
        ...
        for i, node in enumerate(self.nodes):
            ...
            if i == 0:
                # 为GM节点创建零偏差记录
                times = list(range(int(self.simulation_time)))
                results["node_deviations"][i] = [0.0] * len(times)
                results["node_deviations_time_series"][i] = [(t, 0.0) for t in times]
                debug_print(f"节点{i}(主时钟): 时间偏差恒为0")
            else:
                # 非GM节点
...
def analyze_sync_precision(results, threshold_us=1.0):
    ...
    # 计算整体精度（所有非GM节点的最大偏差）
    non_gm_deviations = {k: v for k, v in analysis["max_deviations"].items() if k > 0}
    if non_gm_deviations:
        analysis["overall_precision"] = max(non_gm_deviations.values())
...
def plot_time_deviations(results, start_time=5.0):
    ...
    # 选择要绘制的节点（0, 10, 25, 50, 75, 100）- 包括节点0(GM)
    nodes_to_plot = [0, 10, 25, 50, 75, 100]
...
def create_aligned_time_series_csv(results, filename="aligned_time_deviations.csv"):
    ...
    for node_id in sorted(node_data.keys()):  # 确保节点按ID排序
        # 为每个节点添加一列偏差数据
...
if __name__ == "__main__":
    ...
    # 打印特定节点的详细结果
    selected_nodes = [0, 10, 25, 50, 75, 100]
    print("\n选定节点的详细结果:")
    for node in selected_nodes:
        if node in analysis["max_deviations"]:
            max_dev = analysis["max_deviations"][node]
            sync_prob = analysis["sync_probabilities"].get(node, 1.0)  # 节点0总是同步的
            if node == 0:
                print(f"节点 {node}(主时钟): 最大偏差 = {max_dev * 1e6:.3f} µs, 理想时钟")
            else:
