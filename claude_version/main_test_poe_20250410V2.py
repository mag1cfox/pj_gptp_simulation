...
*  @Description :   IEEE 802.1AS 时间同步仿真 base main_test_poe_20250410.py
*  @FileName:   main_test_poe_20250410V2.py
**************************************
"""
...
class Clock:
    ...
    def __init__(self, node_id=0, initial_time=None, initial_drift_rate=None, max_drift_ppm=50, granularity_ns=8):
        """
        初始化带漂移的时钟
...
class SyncMessage(PTPMessage):
    ...
    def __init__(self, source_port_identity, sequence_id, timestamp=None, two_step_flag=False):
        ...
        参数:
            source_port_identity: 源端口标识
            sequence_id: 序列标识符
            timestamp: 单步模式的源时间戳，两步模式为None
            two_step_flag: 是否为两步同步的一部分
        """
...
class Port:
    ...
    def receive_message(self, message, perfect_time):
        ...
        # 调试输出，确认消息处理
        debug_print(
            f"节点{self.node.node_id}端口{self.port_number}收到{message.message_type}消息，完美时间={perfect_time:.6f}")
    ...
    def process_sync(self, message, reception_time, perfect_time):
        ...
        debug_print(f"节点{self.node.node_id}端口{self.port_number}处理Sync消息: 两步法={message.two_step_flag}")
        ...
        if self.state == PortState.SLAVE:
            ...
            if not message.two_step_flag and message.origin_timestamp is not None:
                ...
                debug_print(f"节点{self.node.node_id}单步同步偏差: {time_deviation * 1e6:.3f} µs")
...
class TimeAwareSystem:
    ...
    def __init__(self, node_id, is_grandmaster=False, priority1=248, priority2=248,
        ...
        参数:
            ...
            phy_jitter_ns: 最大物理抖动（纳秒）
            num_ports: 此节点上的端口数
            two_step_sync: 是否使用两步同步（True）或单步（False）
            two_step_pdelay: 是否使用两步pdelay（True）或单步（False）
        """
        self.node_id = node_id
        ...
        self.sync_interval = 1.0  # syncInterval（1秒）
        self.two_step_sync = two_step_sync  # 是否使用两步同步模式
        self.two_step_pdelay = two_step_pdelay  # 是否使用两步pdelay模式
        ...
        debug_print(f"初始化时间感知系统: 节点{node_id}, GM={is_grandmaster}, 两步同步={two_step_sync}")
    ...
    def process_announce_bmca(self, announce_msg, receive_port):
        """
        处理最佳主时钟算法的Announce消息

        参数:
            announce_msg: 接收到的Announce消息
            receive_port: 接收Announce的端口

        返回:
            如果数据集更改则为True，否则为False
        """
        # 如果我们是主时钟，则跳过BMCA
        ...
        # 如果我们找到了更好的主时钟
        if better_master:
            debug_print(f"节点{self.node_id}发现更好的主时钟: {announce_msg.gm_identity}")
    ...
    def forward_announce(self, announce_message, perfect_time, reception_time):
        ...
        参数:
            announce_message: 要转发的Announce消息
            perfect_time: 转发消息的完美时间
            reception_time: 接收消息的本地时间
...
class IEEE8021ASSimulation:
    ...
    def __init__(self, num_nodes=100, simulation_time=600.0, two_step_sync=True, linear_topology=True):
        ...
        参数:
            ...
            two_step_sync: 是否使用两步同步（True）或单步（False）
            linear_topology: 是否使用线性拓扑或更复杂的拓扑
        """
    ...
    def _setup_linear_topology(self):
        ...
        for i in range(self.num_nodes - 1):
            ...
            if i < self.num_nodes - 2:  # 不是最后一个节点
    ...
    def _process_message_event(self, port, message):
        ...
        debug_print(f"处理消息: 节点{node.node_id}端口{receive_port.port_number}接收{message.message_type}消息")
...
def compare_sync_modes(one_step_results, two_step_results):
    ...
    参数:
        one_step_results: 单步仿真结果
        two_step_results: 两步仿真结果
    """
    ...
    for i, node_id in enumerate(monitored_nodes):
        ...
        if node_id in one_step_results["node_deviations_time_series"]:
            ...
            times = [t for t, _ in time_series]
            deviations = [d * 1e6 for _, d in time_series]  # 转换为微秒
            plt.plot(times, deviations, 'r-', label='单步法', alpha=0.7)
        ...
        if node_id in two_step_results["node_deviations_time_series"]:
            ...
            times = [t for t, _ in time_series]
            deviations = [d * 1e6 for _, d in time_series]  # 转换为微秒
            plt.plot(times, deviations, 'b-', label='两步法', alpha=0.7)

        plt.title(f'节点 {node_id}')
        plt.ylabel('偏差 (µs)')
        plt.grid(True)
...
if __name__ == "__main__":
    ...
    # 配置信息
    print("配置:")
    print(f"- 仿真时间: 60秒 (调试运行)")
    print(f"- 节点数量: 101")
    print(f"- 监控节点: 10, 25, 50, 75, 100")

    # 运行两步法仿真
    print("\n开始两步法仿真...")
    two_step_sim = IEEE8021ASSimulation(num_nodes=101, simulation_time=60.0, two_step_sync=True)
    two_step_results = two_step_sim.run()

    # 检查数据
    two_step_data_count = sum(len(deviations) for deviations in two_step_results["node_deviations"].values())
    print(f"两步法仿真收集了{two_step_data_count}个数据点")

    # 运行单步法仿真
    print("\n开始单步法仿真...")
    one_step_sim = IEEE8021ASSimulation(num_nodes=101, simulation_time=60.0, two_step_sync=False)
    one_step_results = one_step_sim.run()

    # 检查数据
    one_step_data_count = sum(len(deviations) for deviations in one_step_results["node_deviations"].values())
    print(f"单步法仿真收集了{one_step_data_count}个数据点")
    ...
    # 打印两步法摘要
    print("\n两步法同步结果:")
    print(f"使用{two_step_sim.num_nodes}个节点完成{two_step_sim.simulation_time}秒仿真")
    print(f"整体同步精度: {two_step_analysis['overall_precision'] * 1e6:.3f} µs")

    # 打印单步法摘要
    print("\n单步法同步结果:")
    print(f"使用{one_step_sim.num_nodes}个节点完成{one_step_sim.simulation_time}秒仿真")
    print(f"整体同步精度: {one_step_analysis['overall_precision'] * 1e6:.3f} µs")
    ...
    # 只有当数据点足够时才绘图
    if two_step_data_count > 0:
        print("\n绘制时间偏差图 (两步法)...")
        plot_time_deviations(two_step_results)

        if one_step_data_count > 0:
            print("\n比较单步法和两步法同步模式...")
